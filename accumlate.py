...
def accumulate_signal(amplitudes):
    """Accumulates the signal amplitudes."""
    accumulated_amplitudes = []
    current_sum = 0
    for amplitude in amplitudes:
        current_sum += amplitude
        accumulated_amplitudes.append(current_sum)
    return accumulated_amplitudes

def accumulate_and_plot_signal(root):
    ...
    # Accumulate the signal's amplitudes
    accumulated_amplitudes = accumulate_signal(amplitudes)

    # Continuous signal plotting
    cont_window = Toplevel(root)
    cont_window.title("Continuous Signal (Accumulated)")
    fig_cont, ax_cont = plt.subplots()
    cubic_interpolation_model = interp1d(indices_or_freqs, accumulated_amplitudes, kind="cubic")
    x_cubic = np.linspace(min(indices_or_freqs), max(indices_or_freqs), 500)
    y_cubic = cubic_interpolation_model(x_cubic)

    if signal_type == 0:
        ax_cont.plot(x_cubic, y_cubic, label="Continuous Signal (Accumulated)")
    ...
    disc_window.title("Discrete Signal (Accumulated)")
    fig_disc, ax_disc = plt.subplots()

    # Plotting against sample index
    sample_indices = range(len(accumulated_amplitudes))
    ax_disc.stem(sample_indices, accumulated_amplitudes, linefmt='r--', markerfmt='ro', basefmt='b', label="Discrete Signal (Accumulated)")
