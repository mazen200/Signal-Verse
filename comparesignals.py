...
def SignalSamplesAreEqual(file_name,indices,samples):
    ...
    if len(expected_samples)!=len(samples):
        messagebox.showinfo("Alert", "Test case failed, your signal have different length from the expected one")
    ...
    for i in range(len(expected_samples)):
        if abs(samples[i] - expected_samples[i]) < 0.01:
            ...
            messagebox.showinfo("Alert", "Test case failed, your signal have different values from the expected one")
            print("Test case failed, your signal have different values from the expected one") 
            return
    messagebox.showinfo("Gongrats", "Test case passed successfully")
